...
# ==========================================================
# DRAWING HUB (APPENDED SAFELY AT THE BOTTOM)
# ==========================================================
if show_drawing_hub:
    st.write("---")
    st.header(f"📂 Drawing Hub & Uploads: {active_project}")
    ...
    if uploaded_file is not None:
        file_name = uploaded_file.name
        storage_path = f"{active_project}/{engineer_name}_{file_name}"
        ...
        if st.button("📤 Upload Drawing to Project Hub"):
            try:
                ...
                # 4. Write record to database table
                payload = {
                ...
                st.error(f"Upload failed: {e}. Check your Supabase bucket configuration.")

    # DISPLAY DRAWING REGISTRY TABLE
    st.subheader("📁 Active Project Drawing Register")
    try:
        response = supabase.table("drawings_registry").select("*").eq("project_name", active_project).execute()
        drawings = response.data
        ...
        if drawings:
            for d in drawings:
                ...
                with col2:
                    st.write(f"👤 {d['uploaded_by']}")
                with col3:
                    st.markdown(f"[🔗 Open File]({d['file_url']})")
        else:
            st.info("No drawings uploaded to this project yet.")
    except Exception as e:
        st.write("Ready to list files once database is linked.")
